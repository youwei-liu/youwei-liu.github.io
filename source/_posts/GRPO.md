...
- Q1：$\pi_{\theta}$是什么意思？是模型在当前状态下的概率分布吗？
- Q2：在Rollout阶段，用于生成G个响应的prompt数量是如何确定的？是从数据集中随机选择的吗？依据是什么？
- Q3：参数每隔多少步更新一次？更新后是否会产生新的概率分布，即新的$\pi_{\theta}$？
- Q4：其中是否涉及 RM（奖励模型）？ DPO没有 RM。GRPO 是否重新引入了 RM？RM 需要预先训练吗？
- Q5：RM（奖励模型）是如何预先训练的？它的输出只是一个分数吗？人工label是什么样子的？如果以pair出现，且只有good/bad标签，为什么模型能够输出分数？是否可能涉及配比问题
- Q6：关于裁剪（clip）——能理解为 $\rho$ 被限制在 $[1-\epsilon,1+\epsilon]$ 之间吗？直接把 $\rho$ 写为属于 $[1-\epsilon,1+\epsilon]$ 区间可以吗？
- Q7：展开说明为什么PPO需要Critic网络。
- Q8：如果增加clip上界，下界不变，会发生什么？
- Q9：Critic和RM之间是什么关系？
...
**一句话总结：**GRPO/PPO均涉及RM，需要预先训练，且在强化学习阶段冻结参数使用
...
**RM模型：**RM 本质上是一个回归（或排序）模型，通常由 SFT 模型（或更大底座）初始化，去掉最后的 LM Head，接上一个线性层（输出维度为1）。

**人工label：**人工标注的原始数据格式是以Pair（对比对）呈现的，仅包含排序结果，而非分数
